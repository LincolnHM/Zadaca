...
async function renderFormularioReserva() {
  ...
  try {
    const { productos } = await obtenerProductos({ porPagina: 48, orden: 'nombre' });
    mount.innerHTML = `
...
async function enviarReserva(e) {
  ...
  btn.disabled = true;
  try {
    await reservarEnConsolidado(CONSOLIDADO_ID, Number(e.target.id_producto.value), Number(e.target.cantidad.value));
